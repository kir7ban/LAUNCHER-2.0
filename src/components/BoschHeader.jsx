...
/**
 * Corporate Bosch Header (o-header pattern from frontend.kit)
 * Implements the full header (not minimal) with supergraphic, logo, quicklinks, and navigation.
 */
          ...
          <div className="o-header__top">
            ...
            <nav className="o-header__topnav" role="navigation">
              {navigation.map((item, idx) => {
                const hasDropdown = item.isAgentDropdown || item.subNavigation.length > 0;
                return (
                  <div
                    key={idx}
                    className={`o-header__topnav-item ${activeNav === idx ? '-active' : ''}`}
                    onMouseEnter={() => hasDropdown && setActiveNav(idx)}
                    onMouseLeave={() => setActiveNav(null)}
                  >
                    <a href={item.href || '#'} className="o-header__topnav-link">
                      {item.label}
                      {item.isAgentDropdown && (
                        <span className="o-header__topnav-badge">{agents.length}</span>
                      )}
                      {hasDropdown && (
                        <span className="o-header__topnav-arrow">▾</span>
                      )}
                    </a>

                    {/* Agents mega-dropdown */}
                    {item.isAgentDropdown && activeNav === idx && (
                      <div className="o-header__agents-dropdown">
                        <div className="o-header__agents-dropdown-header">
                          <span>All Agents</span>
                          <span className="o-header__agents-dropdown-count">
                            {agents.filter(a => a.status === 'active').length} active
                          </span>
                        </div>
                        {agents.map((agent) => (
                          <a key={agent.id} href="#" className="o-header__agent-item">
                            <span className="o-header__agent-icon">{agent.icon}</span>
                            <div className="o-header__agent-info">
                              <span className="o-header__agent-name">{agent.name}</span>
                              <span className={`o-header__agent-status -${agent.status}`}>{agent.status}</span>
                            </div>
                            <span className="o-header__agent-tasks">{agent.tasksCompleted} tasks</span>
                          </a>
                        ))}
                      </div>
                    )}
                    ...
                    {/* Regular dropdown */}
                    {!item.isAgentDropdown && item.subNavigation.length > 0 && activeNav === idx && (
                      <ul className="o-header__topnav-dropdown">
                        {item.subNavigation.map((sub, si) => (
                          <li key={si}>
                            <a href="#" className="o-header__topnav-dropdown-link">{sub.label}</a>
                          </li>
                        ))}
                      </ul>
                    )}
            ...
            {/* Menu trigger (mobile) */}
            <button
              className="o-header__menu-trigger"
              onClick={() => setMenuOpen(!menuOpen)}
              aria-label="Menu"
            >
              <span className={`o-header__hamburger ${menuOpen ? 'open' : ''}`}>
                <span /><span /><span />
              </span>
            </button>
          </div>
